...
const navigate=useNavigate()
  ...
  const getElektronika=()=>{
    let userRef=collection(db,"elektronika")
  ...
  const getZaryadchiklar=(category)=>{
    let userRef=collection(db,"elektronika")
    getDocs(userRef).then((res)=>{
      let arr=res.docs.map((item)=>{
        return {...item.data(),id:item.id}
...
const ViewCart=()=>{
 ...
 : <p>{x}</p>
        ...
        <div className='title'>
          <div className='left'>
            <h1 style={{fontFamily:"Raleway",}}>Elektronika</h1>
          </div>
          <div className='left-btns'>
           
              <button onClick={()=>getSoatlar("soatlar")} style={{padding:"10px", borderRadius:"5px", border:"none",boxShadow:"0px 4px 12px 0px rgba(54, 58, 69, 0.2)"}}>Soatlar</button>
                <button onClick={()=>getMishkalar("mishkalar")} style={{padding:"10px",border:"none", borderRadius:"5px", boxShadow:"0px 4px 12px 0px rgba(54, 58, 69, 0.2)"}}>Mishkalar</button>
                <button onClick={()=>getPultlar("pultlar")} style={{padding:"10px",border:"none", borderRadius:"5px", boxShadow:"0px 4px 12px 0px rgba(54, 58, 69, 0.2)"}}>Pultlar</button>
                <button onClick={()=>getNaushniklar("naushniklar")} style={{padding:"10px",border:"none", borderRadius:"5px", boxShadow:"0px 4px 12px 0px rgba(54, 58, 69, 0.2)"}}>Naushniklar</button>
                <button onClick={()=>getZaryadchiklar("zaryadchiklar")} style={{padding:"10px",border:"none", borderRadius:"5px", boxShadow:"0px 4px 12px 0px rgba(54, 58, 69, 0.2)"}}>Zaryadchiklar</button>
          ...
          <div className='clear'>
            {
              products.map((item,index)=>(
                <div  key={index} style={{boxShadow:" 1px 2px 16px 3px rgba(54, 58, 69, 0.08)", textDecoration:"none", width:"180px", height:"fit-content", borderRadius:"8px", padding:"15px", background:"#FFFFFF"}}>
